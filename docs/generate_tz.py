...
DOC_CODE = "ИСИП.241П.004-01 ТЗ"
OUT_PATH = "/workspace/docs/TZ_Kritskiy_ISIP-24-1P.docx"
...
def build_title_page(doc):
    ...
    for line in (
        ...
        "Крицкий Кирилл Андреевич",
        "",
        "_________________ / К. А. Крицкий /",
    ):
        if p.runs:
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
...
def build_body(doc):
    ...
    add_body(
        doc,
        "Наименование программы: «Цветочный магазин» "
        "(краткое наименование — ЦМ). Обозначение документа: "
        "ИСИП.241П.004-01 ТЗ. Программа предназначена для учёта "
        "ассортимента, остатков и заказов цветочного магазина. "
        "Область применения — розничная продажа срезанных цветов, "
        "букетов, комнатных растений и сопутствующих товаров. "
        "Объект применения — рабочее место продавца-консультанта "
        "(студент группы ИСИП-24-1П).",
    )
    ...
    add_body(
        ...
        "в сентябре 2026 года. Исполнитель — студент группы ИСИП-24-1П "
        "Крицкий Кирилл Андреевич. Наименование темы: «Разработка программы "
        "«Цветочный магазин»». Шифр темы: ЦМ-2026.",
    )
    ...
    add_body(
        doc,
        "Программа обеспечивает ведение каталога товаров цветочного "
        "магазина, учёт остатков, оформление заказов покупателей "
        "и формирование отчётов по продажам и наличию.",
    )
    ...
    add_body(
        doc,
        "Программа должна выполнять следующие функции: ведение каталога "
        "товаров (название, категория: срезанный цветок, букет, "
        "комнатное растение, упаковка, сопутствующий товар; цвет; "
        "единица измерения; цена; количество на складе; срок годности "
        "для срезанных цветов); добавление, изменение и списание товара; "
        "оформление заказа (дата, покупатель, телефон, состав: товар "
        "и количество, способ получения: самовывоз или доставка, адрес, "
        "дата и время, статус: новый / в работе / выполнен / отменён, "
        "сумма); изменение статуса заказа; поиск товара по названию "
        "без учёта регистра; фильтр по категории, цвету и наличию; "
        "сортировка по названию, цене и остатку; просмотр карточки "
        "товара и заказа; формирование отчётов «Остатки», «Заказы "
        "за период» и «Чек заказа» с выводом на экран, печать и "
        "сохранение в PDF; предупреждение, если остаток товара "
        "ниже заданного минимума; резервное копирование файла базы данных.",
    )
    add_body(
        doc,
        "Входные данные вводятся через формы интерфейса. Выходные данные — "
        "каталог и карточки товара, список заказов, печатные формы "
        "формата A4 и файл базы SQLite. При объёме до 300 товаров "
        "и 200 заказов время открытия каталога — не более 3 с, "
        "оформления заказа — не более 3 с, формирования отчёта — "
        "не более 5 с.",
    )
    ...
    add_body(
        ...
        "названия товара в категории, обязательность названия и цены, "
        "неотрицательный остаток, цена больше нуля, запрет оформления "
        "заказа при недостатке товара на складе. При выполнении заказа "
        "остаток уменьшается автоматически. Отмена заказа возвращает "
        "количество на склад, если заказ ещё не выполнен. Удаление "
        "товара с остатком или заказами — после подтверждения. После "
        "сбоя восстановление — повторным запуском; при повреждении базы "
        "предлагается копия. Время восстановления при наличии копии — "
        "не более 10 минут.",
    )
    ...
    add_body(
        ...
        "категория, заказ, позиция заказа, покупатель. Связь «один "
        "ко многим» между заказом и позициями. Язык реализации — C# "
        "(.NET 8). Среда — Windows 10/11 x64 и .NET 8 Desktop Runtime. "
        "Импорт товаров допускается из CSV (UTF-8): название, категория, "
        "цвет, цена, остаток.",
    )
    ...
    add_body(
        ...
        "ИСИП.241П.004-01, версия, исполнитель и группа. Передача — "
    ...
    add_body(
        ...
        "(название, категория, цена, остаток). Товары с остатком ниже "
        "минимума выделяются красным. В карточке заказа отображаются "
        "состав, сумма и статус. Сохранение — кнопка «Сохранить» "
        "и Ctrl+S. Выход с несохранёнными данными — с запросом "
        "подтверждения. Требования размещения в сети Интернет "
        "не предъявляются (учебная программа).",
    )
    ...
    add_body(
        ...
        "Эффект — отказ от бумажного журнала заказов и ручного подсчёта "
        "остатков цветов. Потребность — один рабочий экземпляр "
        "на точку продаж и один контрольный для преподавателя. "
        "Преимущество перед кассовыми облачными системами — работа "
        "без Интернета и простой набор функций для одной точки; "
        "недостаток — нет онлайн-оплаты и мобильного клиента, "
        "что для учебного изделия допустимо.",
    )
    ...
    add_body(
        ...
        "Исполнитель всех стадий — Крицкий К. А., группа ИСИП-24-1П.",
    )
    ...
    add_body(
        ...
        "15 товаров трёх категорий, 5 заказов (включая доставку и "
        "самовывоз); полный цикл — приход товара, оформление заказа, "
        "уменьшение остатка, смена статуса, отчёты «Остатки» и «Чек» "
        "и резервная копия. Приёмка — при соответствии программы "
        "требованиям настоящего ТЗ, наличии комплекта документации "
        "и успешном запуске на рабочем месте п. 4.4. Отказ в приёмке: "
        "невозможность запуска, потеря данных при сохранении, "
        "оформление заказа сверх остатка, отсутствие отчёта "
        "по остаткам или чека заказа.",
    )
    ...
    add_body(
        ...
        "задание; ПМИ — программа и методика испытаний; ЦМ — настоящая "
        "программа; товар — позиция ассортимента магазина; заказ — заявка "
        "покупателя на товары с указанием способа получения; остаток — "
    ...
    add_sign_line("Исполнитель: студент группы ИСИП-24-1П", space_before=12, keep_next=True)
    add_sign_line("Крицкий Кирилл Андреевич", keep_next=True)
