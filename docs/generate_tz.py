...
DOC_CODE = "ИСИП.241П.002-01 ТЗ"
OUT_PATH = "/workspace/docs/TZ_Panov_ISIP-24-1P.docx"
...
def build_body(doc):
    ...
    add_body(
        doc,
        "Наименование программы: «Каталог компьютерных игр» "
        "(краткое наименование — ККИ). Обозначение документа: "
        "ИСИП.241П.002-01 ТЗ. Программа предназначена для ведения "
        "электронного каталога компьютерных игр с учётом жанра, "
        "игровой платформы и пользовательских рейтингов. Область "
        "применения — учебный и личный учёт игровой коллекции, "
        "подбор игр по жанру и платформе, сравнение оценок. "
        "Объект применения — рабочее место пользователя-коллекционера "
        "(студент группы ИСИП-24-1П).",
    )
    ...
    add_body(
        ...
        "Панов Михаил. Наименование темы: «Разработка программы "
        "«Каталог компьютерных игр» (жанр, платформа, рейтинги)». "
        "Шифр темы: ККИ-2026.",
    )
    ...
    add_body(
        doc,
        "Программа обеспечивает хранение карточек компьютерных игр, "
        "классификацию по жанру и платформе, выставление и просмотр "
        "рейтингов, поиск и отбор записей, формирование списка "
        "и карточки игры.",
    )
    ...
    add_body(
        doc,
        "Программа должна выполнять следующие функции: добавление, "
        "изменение и удаление карточки игры (название, год выпуска, "
        "разработчик, издатель, краткое описание до 1000 символов); "
        "назначение одного или нескольких жанров из справочника "
        "(экшен, RPG, стратегия, симулятор, гонки, спорт, приключение, "
        "хоррор, инди и др.); указание одной или нескольких платформ "
        "(PC, PlayStation, Xbox, Nintendo Switch, мобильные); "
        "выставление пользовательского рейтинга по шкале от 1 до 10 "
        "с шагом 1 и хранение среднего рейтинга по всем оценкам; "
        "поиск по названию без учёта регистра; фильтр по жанру, "
        "платформе, году и диапазону рейтинга; сортировка списка "
        "по названию, году и рейтингу; просмотр карточки игры; "
        "формирование отчёта «Топ игр» (не менее 10 позиций) "
        "с выводом на экран, печать и сохранение в PDF; "
        "резервное копирование файла базы данных.",
    )
    add_body(
        doc,
        "Входные данные вводятся через формы интерфейса. Выходные данные — "
        "список и карточка игры, отчёт «Топ игр» формата A4 и файл "
        "базы SQLite. При объёме до 500 игр время открытия каталога — "
    ...
    add_body(
        ...
        "названия игры в паре «название + год», обязательность названия, "
        "жанра и платформы, диапазон рейтинга 1–10 и года выпуска "
        "1970–текущий год + 1. Удаление карточки — после подтверждения. "
        "После сбоя восстановление — повторным запуском; при повреждении "
        "базы предлагается копия. Время восстановления при наличии копии — "
        "не более 10 минут.",
    )
    ...
    add_body(
        doc,
        "Данные хранятся в файле SQLite. Основные сущности: игра, жанр, "
        "платформа, оценка. Связи «многие ко многим» между игрой и жанром, "
        "игрой и платформой. Язык реализации — C# (.NET 8). Среда — "
        "Windows 10/11 x64 и .NET 8 Desktop Runtime. Импорт списка игр "
        "допускается из CSV (UTF-8): название, год, жанр, платформа, рейтинг.",
    )
    ...
    add_body(
        ...
        "ИСИП.241П.002-01, версия, исполнитель и группа. Передача — "
    ...
    add_body(
        doc,
        "Язык интерфейса — русский. Основной экран — таблица игр "
        "(название, год, жанры, платформы, средний рейтинг). В карточке "
        "игры рейтинг отображается числом и цветом: 8–10 зелёный, "
        "5–7 жёлтый, 1–4 красный. Сохранение — кнопка «Сохранить» "
    ...
    add_body(
        ...
        "Эффект — отказ от разрозненных списков в таблицах и блокнотах "
        "при подборе игр по жанру, платформе и оценке. Потребность — "
        "один рабочий экземпляр и один контрольный для преподавателя. "
        "Преимущество перед веб-каталогами (Steam, Metacritic) — работа "
        "без Интернета и свой набор полей; недостаток — нет онлайн-синхронизации "
        "оценок, что для учебного изделия допустимо.",
    )
    ...
    add_body(
        ...
        "Исполнитель всех стадий — Панов М., группа ИСИП-24-1П.",
    )
    ...
    add_body(
        ...
        "15 игр, 5 жанров, 3 платформ и 20 оценок; полный цикл — "
        "добавление карточки, фильтр по жанру и платформе, выставление "
        "рейтинга, отчёт «Топ игр» и резервная копия. Приёмка — при "
        ...
        "при сохранении, отсутствие фильтра по жанру или платформе, "
        "невозможность выставить рейтинг или сформировать отчёт.",
    )
    ...
    add_body(
        ...
        "задание; ПМИ — программа и методика испытаний; ККИ — настоящая "
        "программа; платформа — аппаратно-программная среда запуска игры "
        "(PC, игровая приставка, мобильное устройство); жанр — тип игрового "
        "процесса; рейтинг — оценка игры по шкале от 1 до 10.",
    )
    add_body(
        doc,
    ...
    add_sign_line("Панов Михаил", keep_next=True)
