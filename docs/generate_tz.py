...
def build_body(doc):
    ...
    add_body(
        doc,
        "Наименование программы: «Электронный журнал учебной группы» "
        "(краткое наименование — ЭЖУГ). Обозначение документа: "
        "ИСИП.241П.001-01 ТЗ. Программа предназначена для учёта текущей "
        "успеваемости и посещаемости в учебной группе специальности 09.02.07 "
        "«Информационные системы и программирование». Объект применения — "
        "группа ИСИП-24-1П; пользователи — куратор, преподаватели и студенты.",
    )
    ...
    add_body(
        ...
        "Долгова София. Наименование темы: «Разработка программы "
        "«Электронный журнал учебной группы»». Шифр темы: ЭЖУГ-2026.",
    )
    ...
    add_body(
        doc,
        "Программа обеспечивает ведение электронного журнала группы: "
        "хранение сведений о студентах и дисциплинах, выставление отметок, "
        "учёт посещаемости и формирование ведомостей.",
    )
    add_subheading(doc, "3.2. Эксплуатационное назначение")
    add_body(
        doc,
        "Программа эксплуатируется в течение учебного семестра на ПЭВМ "
        "под управлением Windows 10/11. Работа — однопользовательская, "
        "с входом по ролям (администратор-куратор, преподаватель, студент). "
    ...
    add_body(
        doc,
        "Программа должна выполнять следующие функции: вход по логину "
        "и паролю с разграничением трёх ролей; ведение списка студентов "
        "группы (ФИО, номер зачётной книжки, статус обучения); ведение "
        "перечня дисциплин и закрепление преподавателя; регистрация "
        "занятий (дата, вид, тема); учёт посещаемости (присутствовал, "
        "«н», «б», «у»); выставление текущих и итоговых отметок "
        "(2–5 либо зачёт/незачёт); расчёт среднего балла и процента "
        "посещаемости; формирование ведомости группы и карточки студента "
        "с выводом на экран, печать и сохранение в PDF; резервное "
        "копирование файла базы данных.",
    )
    add_body(
        doc,
        "Входные данные вводятся через формы интерфейса. Выходные данные — "
        "экранные таблицы, печатные ведомости формата A4 и файл базы SQLite. "
        "При объёме до 40 студентов время открытия журнала — не более 3 с, "
        "формирования ведомости — не более 5 с.",
    )
    ...
    add_body(
        ...
        "номера зачётной книжки, допустимый диапазон отметок и запрет "
        "удаления дисциплины, по которой есть отметки (только архив). "
        "Удаление записей выполняется после подтверждения. После сбоя "
        "восстановление — повторным запуском; при повреждении базы "
    ...
    add_body(
        ...
        "Обслуживание: корректное завершение работы и еженедельная "
        "резервная копия. Персонал: один администратор (куратор) после "
        "инструктажа до 45 минут; преподаватели и студенты — в объёме "
        "своих ролей. Работа с ПЭВМ — не ниже II группы по электробезопасности.",
    )
    ...
    add_body(
        ...
        "в PDF. Для копий — USB-накопитель от 1 Гбайт. Одновременная работа "
        "нескольких экземпляров программы с одним файлом базы не требуется.",
    )

    add_subheading(doc, "4.5. Требования к информационной и программной совместимости")
    add_body(
        doc,
        "Данные хранятся в файле SQLite. Язык реализации — C# (.NET 8). "
        "Среда — Windows 10/11 x64 и .NET 8 Desktop Runtime. Пароли "
        "хранятся в виде хеша с солью. Персональные данные во внешние сети "
        "не передаются. Импорт списка студентов допускается из CSV (UTF-8).",
    )
    ...
    add_body(
        ...
        "ИСИП.241П.001-01, версия, исполнитель и группа. Передача — "
    ...
    add_body(
        doc,
        "Язык интерфейса — русский. Журнал отображается таблицей "
        "(строки — студенты, столбцы — даты). Отметка «2» и неявка «н» "
        "выделяются красным. Сохранение — кнопка «Сохранить» и Ctrl+S. "
        "Выход с несохранёнными данными — с запросом подтверждения. "
        "Требования сертификации СЗИ и размещения в сети Интернет "
    ...
    add_body(
        ...
        "Эффект — сокращение ручного сведения журнала (ориентировочно "
        "около 10 часов за семестр на одну группу). Потребность — один "
        "рабочий экземпляр на группу и один контрольный для преподавателя. "
        "Преимущество перед коммерческими журналами — отсутствие платы "
        "и работы без Интернета; недостаток — нет облачной синхронизации "
        "и мобильного клиента, что для учебного изделия допустимо.",
    )
    ...
    add_body(
        ...
        "Исполнитель всех стадий — Долгова С., группа ИСИП-24-1П.",
    )
    ...
    add_body(
        ...
        "10 студентов, 3 дисциплин и 15 занятий, полный цикл до ведомости "
        "и резервной копии. Приёмка — при соответствии программе требованиям "
        "настоящего ТЗ, наличии комплекта документации и успешном запуске "
        "на рабочем месте п. 4.4. Отказ в приёмке: невозможность запуска, "
        "потеря данных при сохранении, отсутствие ролей, невозможность "
    ...
    add_body(
        ...
        "задание; ПМИ — программа и методика испытаний; ЭЖУГ — настоящая "
        "программа; ПЭВМ — персональная ЭВМ.",
    )
    add_body(
        doc,
